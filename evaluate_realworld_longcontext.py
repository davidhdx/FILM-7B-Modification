...
for model_name in model_names:
    ...
    for pred_filename, label_filename in zip(pred_filenames, label_filenames):
        with open(os.path.join('/content/FILM/real_world_long/results', model_name, pred_filename), 'r', encoding='utf-8') as f_read:
            pred_infos = [json.loads(line) for line in f_read.readlines()]

        with open(os.path.join('/content/FILM/real_world_long/prompts', label_filename), 'r', encoding='utf-8') as f_read:
    ...
    for dataset in datasets:
        ...
        dict_tmp[dataset] = score

    info[model_name] = dict_tmp
